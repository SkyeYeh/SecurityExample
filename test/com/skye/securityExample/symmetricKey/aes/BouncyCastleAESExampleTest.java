 ...
 * @author Skye
 */
public class BouncyCastleAESExampleTest {
	private String data = null;

	@Before
	public void setUp() {
		data = "Hello Bouncy Castle Advanced Encryption Standard.";
	}
	...
	public void testEncrypt() {
		...
		try {
			byte[] key = BouncyCastleAESExample.initKey();
			System.out.println("Key\t：" + Base64.encodeBase64String(key));

			encodeData = BouncyCastleAESExample.encrypt(data.getBytes(), key);
			System.out.println("EncodeData\t："
					+ Base64.encodeBase64String(encodeData));

			decodeData = BouncyCastleAESExample.decrypt(encodeData, key);
