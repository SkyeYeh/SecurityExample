 ...
 * @author Skye
 */
public class BouncyCastleDESExampleTest {
	private String data = null;

	@Before
	public void setUp() {
		data = "Hello Bouncy Castle Data Encryption Standard.";
	}
	...
	public void testEncrypt() {
		...
		try {
			byte[] key = BouncyCastleDESExample.initKey();
			System.out.println("Key\t：" + Base64.encodeBase64String(key));

			encodeData = BouncyCastleDESExample.encrypt(data.getBytes(), key);
			System.out.println("EncodeData\t："
					+ Base64.encodeBase64String(encodeData));

			decodeData = BouncyCastleDESExample.decrypt(encodeData, key);
