 ...
 * @author Skye
 */
public class JavaDESedeExampleTest {
	private String data = null;

	@Before
	public void setUp() {
		data = "Hello Java Triple Data Encryption Standard.";
	}
	...
	public void testEncrypt() {
		...
		try {
			byte[] key = JavaDESedeExample.initKey();
			System.out.println("Key\t：" + Base64.encodeBase64String(key));

			encodeData = JavaDESedeExample.encrypt(data.getBytes(), key);
			System.out.println("EncodeData\t："
					+ Base64.encodeBase64String(encodeData));

			decodeData = JavaDESedeExample.decrypt(encodeData, key);
